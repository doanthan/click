...
| `/account-switch` | `src/app/account-switch/page.tsx` | Switch account (admin-only: search any profile and open the app as that person. `accountSwitchActor` in `src/lib/account-switch-policy.ts` is the single gate - it needs a real `ADMIN_EMAILS` address, refuses `@click.local` QA personas, and expires the grant; the page `notFound()`s for everyone else. The switch runs through the `admin-account-switch` credentials provider in `src/auth.ts` -> `authorizeAccountSwitch` (`src/lib/admin-account-switch.ts`), which re-authorizes server-side so a hand-rolled callback POST is gated too, and requires its `audit_logs` write to commit before the session is issued. A switched session carries `session.impersonation`, renders `AccountViewingBanner`, and is refused by `/qa-unlock` and the admin QA-access toggle until the admin returns to their own account.) |
...
- `api/session-state` (GET - returns `{ version }`, the per-sign-in `sessionVersion` stamped into the JWT. `SessionFreshness` (`src/components/session-freshness.tsx`) polls it on focus, visibility change and a `click:session-changed` storage ping, then reloads when the version moved - so a second tab never keeps rendering the previous account after an account switch or a sign-out. Read-only by contract: it must not refresh a stale session cookie, which is also why `src/proxy.ts` strips Auth.js session refreshes off every proxy response via `withoutSessionRefreshCookies`)
