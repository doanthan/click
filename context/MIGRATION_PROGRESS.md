...
**⏳ 2.5b-iv — wire it (the destructive re-home).** (1) **Read rework — `getProposalsForSession` must go mutual-centric** (FROM `mutual_clicks` LEFT JOIN the pending/accepted proposal): today it's proposal-centric, so a mutual whose only proposal is `declined` vanishes from the list — breaking the decline→open→suggest loop at the list level (the drawer's in-session loop already works off its held entry). (2) Replace `/proposals`' inline `ProposalCard`s with compact "Your clicks" rows (§7 — same layout, state is an accent) that call `openCoordinationDrawer(entry)`; mount `<CoordinationDrawerHost catalogue=…/>`. (3) Wire `?open=<mutualId>` deep-links (auto-open at current step) + retrofit the confirm/propose notification `action_url`s to carry it; reveal-once fires on open. (4) Retire the now-superseded inline card body. Per the migration discipline this is a **destructive change to a working surface** → show the diff before applying.

**⏳ 2.5b-v — §10 acceptance QA (inherently visual — needs the running app).** The 12-point script at 375 / 768 / 1440: reveal fires once & visible (computed `opacity:1`), re-entry never re-fires, steps advance in place with no URL change, both-going + Add-to-calendar, each recovery state, re-render/resize/background-tab torture (no frozen `opacity:0`), `prefers-reduced-motion`, banned-string grep ("match"/"expired"/"missed"/…), and no coordination step reachable as a routed page. Drive in Chrome (dev server `AUTH_URL=http://localhost:3001`, 🧪 test-login switcher) — or hand to Doan.

Then 2.6 UIUX/language → Step 3 teardown. Matching findings (§6) + the 11 "beyond the mechanic" tasks remain out of scope pending Doan.
