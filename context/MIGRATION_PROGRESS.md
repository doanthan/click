...
### ⏳ Next: Step 2.5b — Coordination drawer + one-time reveal
Build the **`CoordinationDrawer`** (`UIUX/COORDINATION_MODAL_SYSTEM.md` v1 §10 = the 12-point acceptance script): ONE stepped modal/drawer over the current page (never routed), a pure projection of `coord_state` (`open→suggest`, `proposed→waiting`, `confirmed_together→both-going ✨`, `dormant`/`released` calm copy). Follow the existing `confirm-dialog`/`login-modal-host` pattern (base state fully visible, no opacity-0 entrance → §5-compliant for free; existing modals already do this). Wire **`seen_at_a/b`** (already in `049`, no migration) for the once-only reveal + a `markMutualSeen` action. Fold in **decline** (`declined` → `coord_state='open'`, mutual untouched) as the drawer's first-class exit, and carry the in-flow report/block (already on the card via SAFE-08). **Invoke the `click-design` skill** before building the UI. Then 2.6 UIUX/language → Step 3 teardown. Matching findings (§6) + the 11 "beyond the mechanic" tasks remain out of scope pending Doan.
