...
> **This is the one meta doc.** Paste the GLOBAL block first, then a per-screen prompt. The canonical per-screen prompts (all in `UIUX/cowork/`): **Landing · HowItWorks_Onboarding · Auth · Dashboard · Discovery_Icons · EventCard · EventDetail · PeopleCard · WhoWasThere · Profile · Booking · MyEvents · ClickMechanic · Notifications · Buttons_Tags**. Retired (do not use): `Click_Design_Prompt_Mockups.md` (v1), `Click_Design_Prompt_ArtDirection.md`, `Click_Design_Prompt_ConsumerCore.md` — their content lives here + in the per-screen prompts + the click-design skill.
