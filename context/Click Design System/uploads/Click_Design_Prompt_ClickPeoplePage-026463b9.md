<!-- Last updated: 2026-07-01 | Revision: v1 (NEW - dedicated render prompt for the "click with someone" PEOPLE PAGE: the daily people pool + click radar + Your clicks. This is a CONVENIENCE PASTE-PROMPT derived from canon, NOT a new source of truth: it stays in sync with `Click_Design_Prompt_ClickMechanic.md` §E [the page] + `Click_Design_Prompt_PeopleCard.md` [the card] + `CLICK_LANGUAGE.md` §5 [the strings]. On ANY conflict, those win and THIS file is corrected. If canon changes, regenerate this prompt. Discovery sub-line = "Three new people, every day." [states the daily refresh; resolves the old eyebrow echo], Cindy 1 Jul.) -->
...
**Discovery eyebrow + sub-line (locked, Cindy 1 Jul):** eyebrow "3 people you might click with today" + sub-line "Three new people, every day." (states the daily refresh - a warm benefit + return hook; resolves the old eyebrow echo since the sub-line no longer repeats "click with"). Approved alternates, one-line swap: "New every day - come back tomorrow for three more." / "New faces every day."
...
2. PEOPLE TO CLICK WITH (the served daily pool - the hero of this page)
   ...
   - Sub-line under the eyebrow, stating the DAILY REFRESH as a warm benefit (so people know to come back): "Three new people, every day." (Do NOT add depleting counters, "X people left", refresh timers/countdowns, or streaks - state the daily refresh, never the internal caps/timing.)
...
1. If you want a different freshness sub-line, swap the one line in section 2 (approved alternates are listed above the prompt) - otherwise leave as is.
