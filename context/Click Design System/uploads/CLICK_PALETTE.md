...
*Outstanding: per-pairing contrast values not yet measured. When the design system is built, run each token pair through a checker and record ratios here. Flagging so it isn't assumed-safe.*
