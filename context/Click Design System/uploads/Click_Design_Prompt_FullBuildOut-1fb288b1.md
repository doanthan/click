...
> **This is the one meta doc.** Paste the GLOBAL block first, then a per-screen prompt. The canonical per-screen prompts (all in `UIUX/cowork/`): **Landing · HowItWorks_Onboarding · Auth · Dashboard · Discovery_Icons · EventCard · EventDetail · PeopleCard · WhoWasThere · Profile · Settings (hosts ProfileEdit) · ProfileEdit · Quiz · Booking · MyEvents · ClickMechanic · Notifications · Buttons_Tags**. Retired (do not use): `Click_Design_Prompt_Mockups.md` (v1), `Click_Design_Prompt_ArtDirection.md`, `Click_Design_Prompt_ConsumerCore.md` — their content lives here + in the per-screen prompts + the click-design skill.
...
- 🔴 PAGE LAYOUT — ONE consistent app container, every signed-in page (Cindy 29 Jun; the #1 cross-page inconsistency). Every signed-in page (Dashboard, Discover, **click-with-someone**, **post-event "Who was there"**, My Events, Profile) uses the SAME content container: **max-width ~1100–1200px, centred on the page with consistent side gutters (~24–32px)**. 🔴 **Within the container, the page TITLE and the content are LEFT-ALIGNED at the container's left gutter — the SAME left edge on every page.** Single-column content (the click-with people list, the profile block, the post-event header) sits **left-aligned** (whitespace falls to the RIGHT) — 🔴 **NEVER a centred-narrow column floating in the middle** (that's why click-with-someone + the post-event page currently look out of place vs Discover/My-Events). Page headers (incl. the post-event "← back / context strip / title") align to this same left gutter — no floating centred header. Mobile (<768): full-width single column with comfortable gutters; the container max-width simply releases. Wide-content pages (Discover grid, My Events) fill the container; narrow-content pages align left within it. (Supersedes any per-page centring — including the earlier "centre the profile block" note; left-align it to the gutter for site consistency, and let the profile's warmth come from real photos.)
...
CATEGORIES — the **canonical 16** (source of truth: `TECH/07_INTEREST_TAGS` §2/§3). 🔴 Use these EXACT categories + their Lucide icons everywhere — NOT ad-hoc tags-as-categories (the live render wrongly used "Pottery", "Coffee", "Markets", which are TAGS, not categories). **ONE consistent icon treatment on EVERY surface** (Discovery filter row · dashboard "browse by category" · onboarding grid): a **Deep-Purple Lucide LINE glyph on a soft Lavender-tint circle + label**; **selected = Deep-Purple FILL circle**. NOT rainbow, never per-category colour. The 16 (07 display order) with their Lucide icon:
1. Wellness `Leaf` · 2. Food & Drink `Wine` · 3. Arts & Crafts `Palette` · 4. Social `Users` · 5. Music `Music` · 6. Fitness & Sport `Dumbbell` · 7. Outdoors `Mountain` · 8. Learning `BookOpen` · 9. Networking `Briefcase` · 10. Dance `Music4` · 11. Creative `Drama` · 12. Lifestyle `Sparkles` · 13. Community `Sprout` · 14. Travel `Map` · 15. Family `Baby` · 16. Dating `Heart` — 🔴 **GATED: the Dating category shows ONLY to dating-intent users** (per 07 + the both-on rule).
🔴 Icon set matches `07` v2: Wellness = **`Leaf`** (Heart is romance-coded AND collides with Dating's Heart); Dance = **`Music4`** (not Zap); Community = **`Sprout`** (not Globe). The single **`Heart`** is reserved for the gated Dating category.
SURFACES: **Discovery** shows "All" + the categories as a wrapping/scrollable filter row; the **dashboard "browse by category"** shows the SAME icons (a curated top row + "see all →" to Discovery); **onboarding** shows all 16 grouped (07 §3 order). Identical icon set + treatment across all three.
People (first name + initial, intent-neutral): Mia, Tom, Priya, Jules, Hassan, Bec.

NAV (web): responsive sticky header — lowercase `click` wordmark left (brand/home); Discover · Dashboard · **✨ click** (the people / "click with someone" destination — lowercase, with the ✨ as a standalone LEADING icon, the only spark in the bar, never merged into the letters) center; "Host an event", a **NOTIFICATIONS BELL** (line icon with an unread DOT that clears on open → a dropdown panel on desktop / sheet on mobile; positive-only, calm, low-volume — full spec `Click_Design_Prompt_Notifications.md`), avatar right. Collapses to a compact header + menu on mobile (the bell stays reachable). Active state Deep Purple. (Optional mobile sticky bottom action bar — Home · Discover · ✨ click · My Events · Profile — is fine as a web pattern; not a native tab bar.)
🔴 **This app nav is for SIGNED-IN app surfaces only.** Logged-out **MARKETING pages — Landing, How-it-works, For-merchants — do NOT show this app nav** (it's clutter + exit paths; removing it lifts conversion). They use a **minimal marketing header**: no app-nav links, no repeated big logo (the hero carries the brand), just the auth entry top-right — **Log in** (quiet) + **Sign up** (Deep-Purple primary) post-launch; pre-launch needs no header at all. (Detail in `Click_Design_Prompt_Landing.md`.)
...
- **Modals / drawers / sheets** (profile centered modal, mutual reveal, suggest-an-event coordination drawer, filters sheet): use a **Close (✕) top-right** (or a sheet grab-handle on mobile); Esc / browser-back / tapping the scrim **dismisses the overlay and returns to the page beneath — never navigates the page away**. Keep the close prominent so users don't reach for browser-back.
...
- CLICK MECHANIC FLOW — post-event prompt ("Did you click with anyone?") → your clicks → MUTUAL REVEAL ("You clicked with [Name]." + "You're both here for [intent]") → suggest-an-event (the no-chat coordination) → closure ("We clicked 👍" → "Love that. That's what Click's for. ✨") → soft-release ("Still out there — if you cross paths again, you can pick it back up." — NEVER loss-framed). The mutual reveal is the most crafted screen. (Detailed: Click_Design_Prompt_ClickMechanic.md.)
- PEOPLE — the "click with someone" page: curated daily set of 3, shown ONE PER LINE (full-width row-cards), the anonymous "click with" surface; intent-neutral; reassurance legible ("🔒 Clicking is anonymous — we'll only show you if it's mutual"). (Detailed: Click_Design_Prompt_PeopleCard.md = the canonical people card; Click_Design_Prompt_ClickMechanic.md §E/§F = the page + profile modal.)
- PROFILE — ONE content model, THREE surfaces: own-profile PAGE (read-only-style preview + "Edit profile"), viewing-someone CENTERED MODAL (public subset + "click with [name]"), and the separate Profile EDIT page (all editable fields + visibility/dating/privacy controls). (Detailed: Click_Design_Prompt_Profile.md = display surfaces; Click_Design_Prompt_ProfileEdit.md = the edit page.)
=== END PHASE 1 ===
...
- SETTINGS — ONE page with sections: **Edit profile · Privacy & visibility · Account · Notifications** (no page-per-setting). Edit profile = profile CONTENT (photos, bio, prompts, multi-select intents, interests; gentle photo prompt, NO "far more clicks"); Privacy & visibility = the "Open to dating" toggle + dating prefs (gated) + "Show me in event attendee lists" + manage-what-others-see; Account = email/password/delete; Notifications = preferences (distinct from the bell panel). "Edit profile" on the profile deep-links to the Edit profile section. NO controls on the profile display. (Detailed: Click_Design_Prompt_Settings.md + Click_Design_Prompt_ProfileEdit.md.)
- CLICK PERSONA QUIZ — full-screen modal from the dashboard checklist + Settings; the personalisation layer onboarding defers (room/social-style/mood/availability/range/optional life-chapter). NO onboarding-owned fields (no intent/dating/interests/identity). (Detailed: Click_Design_Prompt_Quiz.md.)
...
- NOTIFICATIONS — the global bell + panel + push (the click flow leans on it for the mutual moment). (Detailed: Click_Design_Prompt_Notifications.md.) Positive-only, calm, web-only.
