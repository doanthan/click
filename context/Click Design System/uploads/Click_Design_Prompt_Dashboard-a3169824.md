...
- MUTUAL / COORDINATION MOMENT banner — 🔴 **STATE-AWARE (Cindy 28 Jun): it surfaces whichever mutual needs YOUR next action, with the action verb in the button (visibility of system status + one next step).** Never nags about "waiting on them" states — those rest in Your clicks; the dashboard only ever shows YOUR-move moments, one at a time, most-actionable first. Variants (label mirrors the ClickMechanic state map):
  • a fresh mutual (no plan yet) → headline "You clicked with [Name]. ✨" + sub "Skip the small talk - pick something, meet there." + primary **"Suggest a plan →"**.
  • they proposed, your turn → headline "[Name] suggested a plan" + sub "[Event] · [when]" + primary **"See their plan →"**.
  • agreed, your spot to save (paid) → headline "[Name]'s in — save your spot" + sub "[Event] · [when]" + primary **"Save your spot →"**.
  🔴 The ✨ appears ONLY on the fresh-mutual peak headline, as the **Deep-Purple brand glyph** (identical to the nav sparkle) — NOT the raw orange/multicolour emoji (it collides with the Amber status colour), and NOT on the reply/save-spot variants (those aren't peaks). Same Button as above (radius 12, no pill).
