...
- **Away / on another page** → an **immediate notification**: the in-app notification bell lights (a calm dot, not a flood) + an optional push **"It's mutual — you two clicked. ✨"** (locked copy). Opening it → the reveal modal.
- **It also SURFACES warmly where you'll see it next:**
   · **Dashboard (Mode B):** a gentle **"✨ you two clicked with [Name] — suggest a plan →"** moment near the top (below the post-event prompt, above the feed) — the single warmest thing on the dashboard; opens the reveal/coordination. ONE at a time, never a stack.
   · **Your clicks → Live mutuals** (§E3) — the durable home; the mutual rests here as "open" until you propose.
   · The **activity feed** gets a quiet line ("✨ You two clicked with [Name].").
