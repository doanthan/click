...
**Event Card** (`EventCard`) — 🔴 ONE component, identical + SAME fixed size/structure on every surface (dashboard, discovery, landing, my-events). Banner 16:9 (`aspect-ratio`, object-fit cover); **DATE shown** in the eyebrow ("Sat 14 Jun · 2:00pm", not just the weekday); **title = `card-title`, MAX 2 lines + ellipsis (no char cap), title block RESERVES 2 lines so cards align**; tag row = downsized tags, **one line, fit-by-width then "+N" (fewer tags + bigger N when tight), never wrap/stretch the card**; 🔴 **flex text/tag children need `min-width: 0`** (the root cause of titles/tags overflowing instead of truncating); equal-height flex + footer pinned so a row of cards is the same width + height with aligned internal rows; **locked location = ONE line: "Suburb · Distance" + a lock glyph only** (the "venue shown when you RSVP" explanation is the glyph's `aria-label`/tooltip + lives on the detail page — not a visible second row); interest tags **strictly ONE line + "+N" overflow, never wrap**; price; RSVP. Going row centre-aligned. **Equal-height via flex** (content flex-grows, footer pins), responsive grid `repeat(auto-fit, minmax(300px,1fr))` → 1/2/3/3–4 cols, fixed-width horizontal scroll-rows with a peek. Status badge top-left; Save + Share top-right. Venue name/address hidden until booked. Full interest-tag set shows only on the Event Detail page.
