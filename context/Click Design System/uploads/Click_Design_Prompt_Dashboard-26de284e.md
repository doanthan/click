...
8. BROWSE BY CATEGORY — 🔴 give each category a small **canonical category line-icon** (Deep-Purple line glyph on a soft Lavender-tint circle — the ONE category treatment, never rainbow) + label, as compact category tiles (Cindy's question, 27 Jun — **yes, show icons**: icon + label is warmer and far more scannable than a bare pill, and aids recognition). These are navigational CATEGORY entries (icon + label), NOT the neutral interest-tag pill (that's for tags). Tapping → Discovery filtered to that category.
