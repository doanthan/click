...
2. ONE primary CTA: "Request an invite" (invite-only brand — active, exclusive framing beats a passive "list"). On click, reveals a minimal inline form: email + Sydney suburb/postcode ONLY, one button "Request an invite". 🔴 **Out-of-area is NEVER blocked** — a "Somewhere else in Sydney" option / out-of-area postcode still completes the signup and confirms "we'll tell you the moment Click reaches you" (mirrors onboarding). Capture every interested person; never turn traffic away.
3. One quiet line of access/intrigue beneath the CTA: "Invite-only. Launching first in Sydney's inner west and inner east - Newtown, Surry Hills and around. 40+ events already in the works." (Specificity = premium, à la Linear; opportunity-framed, never "don't miss out".) 🔴 **Immediately below it, the out-of-area capture line:** "Somewhere else in Sydney? Join anyway - we'll tell you the moment Click reaches you." — keeps the invite-only pull while widening the door so no signup is lost.
...
=== STATE B — POST-GO-LIVE (minimal FRAME, WARM content — the balance) ===
Same dictionary hero, unchanged (identity anchor + curiosity). 🔴 **The hero stays cool/editorial; the WARMTH comes below it, in the proof layer** — this contrast IS the design (clean frame + warm photography, à la Airbnb/Luma). The live render felt cold because everything was type + GREY placeholder boxes. Fix that with real imagery, not clutter. Add, minimally:
1. WHAT'S ON NEAR YOU THIS WEEK — 3 real event cards (the canonical Event Card), each with **REAL warm-graded ACTIVITY photography — 🔴 NOT grey placeholders** (the grey boxes are exactly why the render reads sterile). Show the EXPERIENCE: clay rising on a wheel, native cocktails being poured, runners at dawn, hands layering moss. Each card: photo · title · suburb · distance (venue hidden) · date · time · one status badge (Free/Trending/Almost full) · aggregate "9 going". Then a quiet "See everything on near you →". NOT a listings wall — 3 cards + a link. This is the PRIMARY warmth + social-proof lever.
2. ONE warm "real life is happening" MOMENT (tasteful, optional) — a single editorial band: a wide, candid, warm-graded photo of an activity IN PROGRESS (a workshop, a table mid-toast, a run setting off) where **people are present but faces are soft / turned / cropped** — conveys fun + people + real life WITHOUT identifiable member faces, the banned "stock diverse-people-laughing" cliché, or revealing the mechanic. One warm line beside it ("Real things, real people — across inner Sydney."). Warmth high, reveal low.
3. Light, HONEST social proof (NO fake testimonials/sponsor logos — banned): specificity + aggregate only — "What's on this week across Newtown, Surry Hills & Redfern" + the cards' "9 going". Real, never staged.
🔴 KEEP THE MAGIC HIDDEN: show the warm OUTCOME (events worth showing up to, people having fun) — NEVER the mechanic (no matching, no faces of who you'd meet, no "likes you"). The curiosity gap (the dictionary + you-don't-see-how) is the hook: warmth pulls the heart, mystery pulls the sign-up.
...
Editorial and asymmetric, not centered-everything. Generous, intentional whitespace (8pt grid) — whitespace is the product looking expensive. **Warmth comes from the IMAGERY, never decoration:** photography (post-launch) = real warm-graded Sydney ACTIVITY shots (clay on the wheel, cocktails pouring, runners at dawn, hands layering moss — the experience), never grey placeholders, never stock, never the banned "diverse-people-laughing" cliché. People MAY appear in the candid activity band, but **faces stay soft / turned / cropped** (members are real — privacy-careful) and never reveal the mechanic. Motion: one restrained moment only — e.g. the definition or the ✨ easing in on load (150–300ms, respect reduced-motion). 
...
[ ] Post-launch WARMTH: real warm-graded ACTIVITY photography on the cards (NOT grey placeholders); optional one candid "real life" band (people present, faces soft/cropped — never stock, never members); honest social proof (specific suburbs + "9 going", no fake testimonials); the mechanic stays hidden
...
- **Warmth vs minimalism (your question, answered):** yes — post-launch should feel warm and alive, but via real **activity photography + honest social proof**, not by adding clutter or revealing the mechanic. The hero stays cool/editorial (curiosity); the warmth sits below it in the proof layer. That clean-frame + warm-content contrast is how Airbnb/Luma feel premium AND warm. We show events worth showing up to and the *sense* of people having fun — never identifiable members, never the matching. Warmth pulls the heart; the hidden magic pulls the sign-up.
- **🔵 NEXT PHASE — site-wide warm-photography pass.** The single biggest warmth lever (real warm-graded activity photos replacing grey placeholders) should extend across the whole site: Discovery cards, the Event Detail hero, the dashboard "what's on" strips, Who-was-there. That's an art-direction/photography pass for a later phase (flagged, not done now) — say the word and I'll spec it across all surfaces.
