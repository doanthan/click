...
- ✨ **Mutual** (the key one): "✨ You clicked with Mia — suggest a plan." → on first tap, shows the **mutual reveal ONCE**; thereafter opens the **coordination drawer at its current step** (propose → waiting → both-going) — **never re-fires the reveal** (ClickMechanic §B). This is the row the whole bell exists for.
