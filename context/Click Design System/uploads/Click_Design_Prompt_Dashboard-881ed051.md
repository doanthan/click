...
- POST-EVENT PROMPT: the two actions **STACK full-width** on mobile (primary "See who was there" above, "Maybe later" quiet below) — never side-by-side (the live render wrapped "See who was there" onto two lines). Subline (locked, tightened): "Click anyone worth a second hang - we'll do the rest." (drop the filler "actually"). Desktop may sit the primary + quiet "Maybe later" inline, right-aligned.
...
- MUTUAL / COORDINATION MOMENT banner — 🔴 **STATE-AWARE (Cindy 28 Jun): it surfaces whichever mutual needs YOUR next action, with the action verb in the button (visibility of system status + one next step).** Never nags about "waiting on them" states — those rest in Your clicks.
  🔴 **ONE banner COMPONENT, used for the post-event prompt AND every coordination state (consistency — research: never stack banners; one consistent component + hierarchy).** Layout, identical every time: a **Lavender-tint rounded card (~16px radius, ~20–24px padding)** · an **icon-circle left** (~40px, Lavender-200 fill, Deep-Purple line glyph) · a middle column **eyebrow (`micro`, Slate) → title (Poppins ~h3 20/28, Ink) → one subline (`body-sm`, Slate)** · the **primary action on the RIGHT, vertically centred** (desktop) / stacked full-width below (mobile). Same Button (radius 12, no pill) everywhere. Finish-setting-up is NOT this component (it's the quieter completion card).
  Variants (content only; label mirrors the ClickMechanic state map):
  • **post-event prompt** → eyebrow "LAST NIGHT · [EVENT]" · title "Did you click with anyone?" · sub "Click anyone worth a second hang - we'll do the rest." · "See who was there" + quiet "Maybe later".
  • **fresh mutual (no plan yet)** → eyebrow "IT'S MUTUAL" · title "You clicked with [Name]. ✨" · sub "Find something you'd both enjoy and meet there." · primary **"Suggest a plan →"**.
  • **they proposed, your turn** → eyebrow "FROM [NAME]" · title "[Name] suggested a plan" · sub "[Event] · [when]" · primary **"See their plan →"**.
  • **agreed, your spot to save** → eyebrow "YOUR PLAN WITH [NAME]" · title "[Name]'s in - RSVP to lock it in" · sub "[Event] · [when]" · primary **"RSVP →"** (routes to the event detail page, §C3).
  🔴 **MORE THAN ONE mutual needs you → ONE consolidated banner, never a stack (Cindy 28 Jun; research: aggregate, don't pile banners).** When 2+ mutuals are waiting on your action: eyebrow "YOUR CLICKS" · title "[Name] and [Name] are waiting on you ✨" (or "3 of your clicks need a plan" for >2) · sub "Pick up where you left off." · primary **"See your clicks →"** that routes to the **click / people page** (Your clicks) where each is listed with its own state button. The dashboard shows AT MOST ONE coordination banner; single = the specific action, multiple = divert to the hub.
  🔴 The ✨ appears ONLY on the fresh-mutual peak headline (and the consolidated multi-mutual title), as the **Deep-Purple brand glyph** — NOT the orange emoji, and NOT on the reply/RSVP variants (not peaks). Order when both a post-event prompt and a coordination moment exist: post-event prompt first, then ONE coordination banner.
