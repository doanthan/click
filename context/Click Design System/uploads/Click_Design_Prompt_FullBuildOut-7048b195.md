...
- EVENT DETAIL — locked (suburb-only + FOMO + capacity bar + status tags) / unlocked (full address + map + manage +1s + cancel/refund) / waitlist (full); share on all; NO click button; aggregate social proof only (never a named person tagged with intent); on-brand photo nudge. CTA = RSVP / Join waitlist / You're going. (Detailed: Click_Design_Prompt_EventDetail.md; booking flow: Click_Design_Prompt_Booking.md.)
