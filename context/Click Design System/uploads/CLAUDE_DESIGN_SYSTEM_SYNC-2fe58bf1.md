...
| Your-clicks cards show pills: **"You two clicked" / "You're both going" / "We clicked 🔥" / "Released gently"** | **No name-adjacent state pill** (Cindy 28 Jun). Section header (Live mutuals / Plans / Past clicks) + a quiet state accent + the action carry the state; card leads with the NAME. Remove those pills and any ✨ on a list card. "We clicked 👍" stays only as the closure ACTION; soft-released cards carry no marker; **"Released gently" is never a user-facing label.** |
...
## 28 Jun 2026 — LIVE DESIGN-SYSTEM AUDIT (Cindy walked the project's Design System pages)
The README-replace (above) fixes the root cause, but these design-system **frames/pages** carry their own stale content and must be corrected directly (the README won't redraw them):

🔴 **Critical:**
- **Category icons page = WRONG SET.** It shows ad-hoc tags-as-categories (Pottery & ceramics · Run clubs & fitness · Wine & bars · Cooking · Live music · Art & craft · Wellness · Trivia & games · Outdoors · Markets · Coffee · Workshops). Replace with the **canonical 16** (07_INTEREST_TAGS v2 / changelog v36, in order): Wellness `Leaf` · Food & Drink `Wine` · Arts & Crafts `Palette` · Social `Users` · Music `Music` · Fitness & Sport `Dumbbell` · Outdoors `Mountain` · Learning `BookOpen` · Networking `Briefcase` · Dance `Music4` · Creative `Drama` · Lifestyle `Sparkles` · Community `Sprout` · Travel `Map` · Family `Baby` · Dating `Heart` (gated to dating-intent). One treatment (Deep-Purple line on Lavender-tint circle; selected = Deep-Purple fill). The dashboard render already uses the 16 — this page contradicts it.
- **MutualCard headline "You clicked with each other. ✨" → "You clicked with [Name]."** (retired; CLICK_LANGUAGE v13/v30). Also "pre-event routes to the meeting point" → routes to **"suggest a plan"** / the coordination drawer ("meeting point" is stray terminology).
- **Type sample H2 "Who'd you click with?" → "Did you click with anyone?"** (retired; CLICK_LANGUAGE v10).
- **Error red → use the canonical `#B5362F`** (added to CLICK_PALETTE v2, 28 Jun — AA on cream). 🔴 **Replace the project-invented `#C0504A`** (it failed AA at 4.34:1). Use error red for genuine errors / destructive ONLY (form validation, destructive-confirm) — as text, a field border, or a soft ~10–12% tint banner; NEVER a status badge, NEVER a CTA, NEVER decoration. Drop the rest of the "Semantic colors" Success/Warning/Error/Info block — Success = Sage, Warning = Amber, Info = Teal already live in the status palette (badges only, sparingly); a separate big semantic-swatch section invites misuse.
- **Corner-radii scale (6/10/14/20/28/pill) is missing the values the components use.** Add/align: **buttons 12 · badges 8 · cards 16–20 · inputs 12 · tags/avatars full.** The radii tokens must match the component specs.

🟠 **Medium:**
- **Tag size → 22px height / 12px label / 8px padding / 6px gap** everywhere (changelog v47→v49; the pages still show the old ~28/13). One line + measured "+N", never wrap/stretch the card.
- **EventCard: venue NAME hidden until booked.** The "venue + suburb" note must become locked = "suburb · distance + lock glyph", unlocked = venue name. Never the venue name on a locked card.
- **Event-card capacity badge = "Almost full" (Coral), not "2 spots left".** The exact count lives on Event Detail only.
- **"Mutual" is NOT a status badge.** Remove it from the Lavender & Status badge row — the mutual state is the click button "clicked ✨".
- **✨ = the one Deep-Purple brand glyph, never the orange emoji** (changelog v44); peaks only.

🟢 **Minor:**
- Body type sample: use hyphens not em-dashes; and don't over-explain the safety ("anonymous, mutual only") — §5b says lead with desire, let safety be felt.
- Confirm the "Sand" ramp is only subtle warm grounds, not a second brand colour (cream #F9F6F0 is the canvas).

**Otherwise the system is a faithful mirror** — Buttons, People Card states, Badges-as-rounded-rect, Category-icon *treatment*, the editorial 'click' moment (Poppins purple, no serif), Shadows, the privacy toggle, IntentLine, the avatars-meet — all correct and on-brand. No real duplication beyond the stale category set.
