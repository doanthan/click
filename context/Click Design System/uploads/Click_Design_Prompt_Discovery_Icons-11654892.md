...
CATEGORIES — use the **canonical 16** (source of truth: `TECH/07_INTEREST_TAGS` §2/§3). 🔴 These are the real categories - do NOT use ad-hoc tags ("Pottery", "Coffee", "Markets") as categories; those are TAGS within a category. The 16, in 07 display order, each with its Lucide icon:
1. Wellness `Leaf` · 2. Food & Drink `Wine` · 3. Arts & Crafts `Palette` · 4. Social `Users` · 5. Music `Music` · 6. Fitness & Sport `Dumbbell` · 7. Outdoors `Mountain` · 8. Learning `BookOpen` · 9. Networking `Briefcase` · 10. Dance `Music4` · 11. Creative `Drama` · 12. Lifestyle `Sparkles` · 13. Community `Sprout` · 14. Travel `Map` · 15. Family `Baby` · 16. Dating `Heart` — 🔴 **GATED: only shown to dating-intent users.**
🔴 Icon refinements: 07 lists `Heart` for Wellness, but it's romance-coded AND collides with Dating's Heart - use **`Leaf`** for Wellness and reserve the one **`Heart`** for the gated Dating category. (07's Dance `Zap` reads as energy/lightning → use `Music4`; Community `Globe` → `Sprout`.)
On Discovery the category row begins with **"All"** (selected by default) then the 16; the dashboard "browse by category" shows the SAME glyphs (curated top row + "see all →"); onboarding shows all 16. Identical treatment on all three.
