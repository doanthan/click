...
- **Chips/tags:** Material 3 chips are **32px tall with ~8px between chips**; chips are deliberately lighter than buttons. → Click tags at **24px** (label 12px) read as ~55% of a 44px button — clearly a tag, not a button (downsized site-wide 28 Jun for a calmer, denser read).
...
PALETTE (locked, exact): Deep Purple #3B2F81 (primary + selected ONLY), Lavender #C8B8F8 (soft wash), Cream #F9F6F0 (canvas), Ink #1C1830 (text — never pure black), Slate #6B6580 (muted), Mist #E8E4F0 (borders/disabled). Status (BADGES ONLY — never buttons or category tags): Coral #E8674C, Amber #E0A33A, Sage #5B8C6E, Teal #2E7D8A. Button hover/pressed purples: hover #332873 (~8% darker), pressed #2A2160 (~12% darker).
...
- Resting (every tag, every surface, identical — 🔴 Cindy 28 Jun DOWNSIZE, apply SITE-WIDE): white fill, 1px Mist #E8E4F0 hairline, Ink text. NO dot. **Height 24px (dense 22px), full pill radius, label 12px weight 500, horizontal padding 10px (dense 8px), 6px gap between tags.** (Was 28 / 13 / 12-pad / 8-gap — now smaller + lighter EVERYWHERE: event cards, people cards, Your-clicks cards, dashboard strips, landing, discovery, onboarding/filters. Buttons stay 44px, so a tag reads as ~55% of a button — unmistakably a tag.)
- Selected (onboarding grid / filters only): Deep Purple #3B2F81 fill, cream text — **NO check/tick** (the purple fill IS the selected signal; Cindy 27 Jun). This is the ONLY time a tag goes purple. For accessibility add `aria-pressed`/`aria-selected` on the chip so screen readers convey the state without a visible tick (the full fill is a strong visual change, not a hue-only cue, so it's safe for sighted users). Selectable tags keep a ≥24px hit target (height 24 meets WCAG 2.2; expand the tap area with padding if needed, not the visible pill).
...
- 🔴 **ONE LINE, NEVER WRAP, NEVER STRETCH THE CARD (Cindy 28 Jun — the render let a tag break the card width).** The tag row is constrained to the content width and clipped to a SINGLE line. Render as many WHOLE tags as fit, then collapse the remainder into a neutral **"+N"** chip (e.g. "Plants · Craft · +2"). On a narrower card, show fewer tags and a bigger N — the row must NEVER wrap to a second line, NEVER horizontally scroll on a card, and a long tag must NEVER push the card wider. Tags don't shrink their font to fit; the count adapts, not the size. Same behaviour on every surface.
...
[ ] Tags read as ~24px (label 12px) vs 44px buttons — clearly lighter/smaller; one line + "+N", never wrap/stretch the card
