...
- POST-EVENT PROMPT: the two actions **STACK full-width** (primary "See who was there" above, "Maybe later" quiet below) — never side-by-side (the live render wrapped "See who was there" onto two lines).
...
- By DEFAULT the card is SLIM: header + the pre-filled progress bar + "X of 5" + ONLY the **single NEXT incomplete step** (its title + one-line value + CTA), then a quiet **"see all →" / chevron** to expand the remaining steps inline. Do NOT render all 5 rows expanded by default — that full list is the bulk Cindy flagged.
...
- At 100% the card shows the one celebration then **collapses/dismisses**. (Mode B already uses the slim strip — Mode A now defaults to a similarly compact card, expandable.)
...
- Each BAR = ONE row: a small line-icon (people/spark) + the social-proof line + the event name + a chevron / tap-to-event. Examples:
