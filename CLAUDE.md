...
| `/test-click` | `src/app/test-click/page.tsx` (interactive explainer of the **Click mechanic** — answers the "what's happening at each step?" confusion. Server shell (hero + outcomes + TL;DR closer band) mounts the `ClickWalkthrough` client island: a state-machine overview diagram → the two entry-points fork → a 5-step interactive stepper (private pending → mutual → proposal → confirm → both going), each step rendering a **faithful static clone** of the real surface (`click-with-someone-user-card` / `post-event-click-card` / the rose dashboard mutual card / `proposal-card`) with a Trigger · You see · They see · Clock annotation. No DB reads, no server actions — demo data only; degrades to a stacked static walkthrough with no JS. Sibling to the public `/how-it-works`. Keep all claims diffed against the real mechanic in `event-repository.ts`) |
